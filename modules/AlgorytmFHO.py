...
class CalculationFHO(CalculationInterface):
    # Start dynamizing
    ...
    def calculate(self, data_path: str):
        ...
        obj_values = np.array([objective(data_path, X[s], False) for s in range(params.n_s)])
        ...
        while run < params.iterations:
            ...
            if self.stop:
                ...
                self.finished.emit(X[np.argmin(obj_values)])
                return X[np.argmin(obj_values)]

            self.emitProgress.emit(X[np.argmin(obj_values)])
            ...
            obj_values = np.array([objective(data_path, X[s], False) for s in range(len(X))])
...
# print(solve)
# print(objective(data_path, solve, False))
...
#     objective_value = objective(data_path, solve, False)
...
#     #print(f"Started solving for {data_path}")
#     start_time = time.time()
#     solve = fire_hawk_optimization(params, objective)
#     end_time = time.time()
#
...
#     #print(solve)
#     print(f"{objective(data_path, solve, False):.3e}")
#     print(f"Stopped solving for {data_path}. Time taken: {int(minutes)} minutes and {seconds:.3f} seconds")
#     print("-" * 50)
#
#
# # Uruchomienie obliczeń dla każdego pliku
# for data_path in data_paths:
#     solve_for_file(data_path)
